...
uses Windows, SysUtils, Graphics, Dialogs, akSysCover, Controls;
...
type
  ...
  THtmlHelp = function(hwndCaller: THandle; pszFile: PChar; uCommand:
    cardinal; dwData: longint): integer; stdcall;

function HtmlHelp(hwndCaller: THandle; pszFile: string; uCommand:
  cardinal; dwData: longint): integer;
...
function HtmlHelp;
const
  HH_CTRLOCX_ERROR = 'HHCtrl.ocx library not found.'#13#10 +
    'Unable to display the help file.'#13#10#13#10 +
    'Please press OK button to update your help viewer.';
begin
  Result := -1;
  ...
  if (not Assigned(HtmlHelpProc)) then begin
    ...
    if (MessageDlg(HH_CTRLOCX_ERROR, mtError, [mbOk, mbCancel], 0) = mrOK) then begin
      TryOpenUrl('http://msdn.microsoft.com/library/default.asp?url=/library/en-us/htmlhelp/html/hwMicrosoftHTMLHelpDownloads.asp');
    end;
  end else
...
initialization
  HtmlHelpProc := nil; HHCtrlHandle := 0;
